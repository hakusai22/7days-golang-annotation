...
// Call 首先我们封装了结构体 Call 来承载一次 RPC 调用所需要的信息
type Call struct {
	Seq           uint64
...
type Client struct {
	...
	closing  bool             // closing 和 shutdown 任意一个值置为 true，则表示 Client 处于不可用的状态，但有些许的差别，
	shutdown bool             // closing 是用户主动关闭的，即调用 Close 方法，而 shutdown 置为 true 一般是有错误发生。
}
